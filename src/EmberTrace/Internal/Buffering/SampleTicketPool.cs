...
internal sealed class SampleTicketPool
{
    ...
    public bool TryGetSlot(int id, out SampleSlot slot) => _slotsById.TryGetValue(id, out slot);

    public long RentBlock(int slot) => Interlocked.Add(ref _issued[slot], BlockSize) - BlockSize;
}
