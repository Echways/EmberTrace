...
`RestrictToLoopback` также отклоняет запросы, на которых остался заголовок `X-Forwarded-For` или
`Forwarded`: за reverse proxy на том же хосте любой запрос приходит с `127.0.0.1`, и адрес сокета
сам по себе ничего не доказывает. Вызовите `app.UseForwardedHeaders()` до эндпоинта, чтобы заголовки
прокси были обработаны, или защитите эндпоинт через `ApiKey` либо `AuthorizationPolicy`.
