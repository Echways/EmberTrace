...
- Интерфейс — единственный напрямую реализованный, не считая `IDisposable` и `IAsyncDisposable`, либо
  указанный в `[Trace(Interface = typeof(IX))]`. Всё остальное — `ETG014`.
- Члены, унаследованные интерфейсом, тоже реализуются. `Dispose` и `DisposeAsync` никогда не
  трассируются: если класс освобождаемый, декоратор тоже освобождаемый и пробрасывает освобождение,
  поэтому контейнер, владеющий декоратором, освобождает и сервис за ним.
